#pragma once

#include "Component.h"
#include "Bone.h"

BEGIN(Engine)

class ENGINE_DLL CModel final : public CComponent
{
public:
	enum TYPE { TYPE_NONANIM, TYPE_ANIM, TYPE_END };

private:
	CModel(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CModel(const CModel& Prototype);
	virtual ~CModel() = default;

public:
	vector<class CMesh*>&		Get_Meshes() { return m_Meshes; }
	vector<MESH_MATERIAL>&		Get_Material() { return m_Materials; }
	vector<class CBone*>&		Get_Bones() { return m_Bones; }
	vector<class CAnimation*>&	Get_Animations() { return m_Animations; }
	vector<vector<_uint>>&		Get_KeyFrameIndicies() { return m_KeyFrameIndices; }

	_uint					Get_NumMeshes() const { return (_uint)m_Meshes.size(); }
	_uint					Get_NumBone(_uint iIndex) const;

	TYPE					Get_ModelType() const { return m_eType; }

	_int					Get_BoneIndex(const _char* pBoneName) const;
	_matrix					Get_BoneCombindTransformationMatrix(_uint iBoneIndex) const { return m_Bones[iBoneIndex]->Get_CombinedTransformationMatrix(); }
	void					Set_BoneCombindTransformationMatrix(_uint iBoneIndex, _matrix CombinedTransformationMatrix) { m_Bones[iBoneIndex]->Set_CombinedTransformationMatrix(CombinedTransformationMatrix); }

	_matrix					Get_BoneTransformationMatrix(_uint iBoneIndex) const { return m_Bones[iBoneIndex]->Get_TransformationMatrix(); }
	void					Set_BoneTransformationMatrix(_uint iBoneIndex, _matrix TransformationMatrix) { m_Bones[iBoneIndex]->Set_TransformationMatrix(TransformationMatrix); }

	const _float4x4*		Get_BoneCombindTransformationMatrix_Ptr(const _char* pBoneName) const { return m_Bones[Get_BoneIndex(pBoneName)]->Get_CombinedTransformationMatrix_Ptr(); }
	const _float4x4*		Get_BoneCombindTransformationMatrix_Ptr(_uint iBoneIndex) const { return m_Bones[iBoneIndex]->Get_CombinedTransformationMatrix_Ptr(); }
	
	_uint					Get_AnimationIndex(const _char* pAnimationmName, _double SpeedRatio = 1.0) const;

	_uint					Get_CurrentAnimationIndex() { return m_iCurrentAnimIndex; }
	_char*					Get_CurrentAnimationName();

	_bool					Get_IsEnd_Animation(_uint iAnimationIndex);

	_uint					Get_Frame(_uint iAnimIndex);


public:
	virtual HRESULT Initialize_Prototype(TYPE eType, const _char* pModelFilePath, _fmatrix PreTransformMatrix);
	virtual HRESULT Initialize(void* pArg) override;
	virtual HRESULT Render(_uint iMeshIndex);

public:
	_vector Play_Animation(_float fTimeDelta, _uint iRootBoneIdex = 2);
	_uint Find_Mesh(const _char* szMeshName);
	HRESULT Add_Texture_to_Material(const _tchar* pTextureFilePath, TEXTURE_TYPE eMaterialType, _uint iMeshIndex);

	void Change_PartInfo(CModel* pMeshModel, CModel* pAnimModel = nullptr );

	class CBone* Find_Bone(string strBoneName) const;
	void		Add_Bone(class CBone* pBone);
	void Set_Bones(vector<class CBone*>& Bones);
	void Set_Animations(vector<class CAnimation*>& Animations, vector<vector<_uint>>& KeyFrameIndices);

	class CTexture* Find_Texture(_uint iMeshNum, TEXTURE_TYPE eMaterialType);

public:
	HRESULT Bind_Material(class CShader* pShader, _char* pConstantName, TEXTURE_TYPE eMaterialType, _uint iMeshIndex, _uint iTextureIndex = 0);
	HRESULT Bind_MeshBoneMatrices(class CShader* pShader, const _char* pConstantName, _uint iMeshIndex, _float4x4* BoneMatrix = nullptr);


private:
	TYPE							m_eType = { TYPE_END };

private: /* 메시의 정보를 저장한다. */
	_uint							m_iNumMeshes = { 0 };
	vector<class CMesh*>			m_Meshes;
	_float4x4						m_PreTransformMatrix = {};

private:
	_uint							m_iNumMaterials = { 0 };
	vector<MESH_MATERIAL>			m_Materials;

private:
	vector<class CBone*>			m_Bones;

private:
	_bool							m_isLoop = { false };
	_uint							m_iCurrentAnimIndex = { 0 };
	_uint							m_iNumAnimations = { 0 };
	vector<class CAnimation*>		m_Animations;

	/* 현재 애니메이션의 재생 위치. */
	_double							m_CurrentTrackPosition = {};
	_double							m_ChangeTrackPosition = {};

	_int							m_iCurrentFrame = { 0 };

	/* 각 애니메이션당 각 채널들의 현재 키프레임인덱스 */
	vector<vector<_uint>>			m_KeyFrameIndices;

	_bool							m_isChangeAni = { false };
	CHANGEANIMATION_DESC			m_tChangeDesc = {};

	_float3							m_vPreRootPos = {};
	_uint							m_iRootBoneIndex = {};

	_bool*							m_isEnd_Animations;

public:
	void		SetUp_Animation(_uint iAnimationIndex, _bool isLoop = false);
	void		Reset_Animation(_uint iAnimationIndex);

	HRESULT     SetUp_NextAnimation(_uint iNextAnimationIndex, _bool isLoop = false, _float fChangeDuration = 0.2f, _uint iStartFrame = 0);

public:
	HRESULT	Ready_Meshes(HANDLE* pFile);
	HRESULT Ready_Materials(HANDLE* pFile, const _char* pModelFilePath);
	HRESULT Ready_Bones(HANDLE* pFile, _int iParentBoneIndex);
	HRESULT Ready_Animations(HANDLE* pFile);

public:
	static CModel* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, TYPE eType, const _char* pModelFilePath, _fmatrix PreTransformMatrix = XMMatrixIdentity());
	virtual CComponent* Clone(void* pArg) override;
	virtual void Free() override;
};

END