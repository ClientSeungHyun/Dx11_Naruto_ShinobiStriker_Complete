...
CState_Custom_Amateras::CState_Custom_Amateras(CFsm* pFsm, CPlayer* pPlayer)
	:CState{ pFsm }
	, m_pPlayer{ pPlayer }
{
}

HRESULT CState_Custom_Amateras::Initialize(_uint iStateNum)
{
...
HRESULT CState_Custom_Amateras::Start_State(void* pArg)
{
...
void CState_Custom_Amateras::Update(_float fTimeDelta)
{
	...
	{
		if (KEY_NONE(KEY::NUM1))
		{
			m_pPlayer->Get_SoundCom()->Play3D(TEXT("Voice_Amateras.wav"), g_fVolume + 0.2f);
			m_pPlayer->SetUp_NextAnimation(m_iAnimation_Amateras_End, false);
			dynamic_cast<CPlayer_Custom*>(m_pPlayer)->Create_Amateras();
		}
	}
}

void CState_Custom_Amateras::End_State()
{
	m_pPlayer->Set_IsInvincible(false);
	//m_pGameInstance->Stop_Sound(SOUND_SKILL);
}

CState_Custom_Amateras* CState_Custom_Amateras::Create(CFsm* pFsm, CPlayer* pPlayer, _uint iStateNum)
{
	CState_Custom_Amateras* pInstance = new CState_Custom_Amateras(pFsm, pPlayer);
...
void CState_Custom_Amateras::Free()
{
	__super::Free();
}
