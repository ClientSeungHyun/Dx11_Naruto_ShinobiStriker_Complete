#include "stdafx.h"
#include "UI_BossHP.h"

#include "Monster.h"
#include "GameInstance.h"

CUI_BossHP::CUI_BossHP(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CUIObject{ pDevice, pContext }
{
}

CUI_BossHP::CUI_BossHP(const CUI_BossHP& Prototype)
	: CUIObject{ Prototype }
{
}

HRESULT CUI_BossHP::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CUI_BossHP::Initialize(void* pArg)
{
	HPUI_DESC* pDesc = static_cast<HPUI_DESC*>(pArg);

	m_pBossHP = pDesc->pBossHP;
	m_fMaxHP = *m_pBossHP;
	m_pMonster = pDesc->pMonster;

	/* 직교퉁여을 위한 데이터들을 모두 셋하낟. */
	if (FAILED(__super::Initialize(pDesc)))
		return E_FAIL;

	if (FAILED(Ready_Components()))
		return E_FAIL;

	if (m_pMonster->Get_MonsterID() == KURAMA)
		m_eIconType = ICON_KURAMA;
	else
		m_eIconType = ICON_SUSANOO;

	m_iDepth = 2;
	m_TextCoord = _float2(0.f, 1.f);
	m_fAlpha = 0.f;

	m_isActive = true;

	return S_OK;
}

void CUI_BossHP::Priority_Update(_float fTimeDelta)
{

	for (auto& Child : m_UIChilds)
	{
		Child->Priority_Update(fTimeDelta);
	}
}

void CUI_BossHP::Update(_float fTimeDelta)
{
	if (m_isAppear)
		Appear(fTimeDelta);
	else if (m_isDisappear)
		Disappear(fTimeDelta);
	else
	{
		m_TextCoord.x = (*m_pBossHP) / m_fMaxHP;
		if (m_TextCoord.x <= 0.f)
			m_TextCoord.x = 0.f;
		else if (m_TextCoord.x >= 1.f)
			m_TextCoord.x = 1.f;
	}

	if (m_isDamaged)
	{
		m_fDamagedTime += fTimeDelta;
		if (m_fDamagedTime > 0.8f)
		{
			m_fAlpha = 0.f;
			m_isDamaged = false;
		}
		else if (m_fDamagedTime < 0.3f)
			m_fAlpha += 10.f * fTimeDelta;
		else if (m_fDamagedTime >= 0.3f)
			m_fAlpha -= 10.f * fTimeDelta;

		if (m_fAlpha <= 0.f)
			m_fAlpha = 0.f;
		else if (m_fAlpha >= 1.f)
			m_fAlpha = 1.f;
	}
}

void CUI_BossHP::Late_Update(_float fTimeDelta)
{
	__super::Late_Update(fTimeDelta);

	for (auto& Child : m_UIChilds)
	{
		Child->Late_Update(fTimeDelta);
	}

	m_pGameInstance->Add_RenderObject(CRenderer::RG_UI, this);
}

HRESULT CUI_BossHP::Render()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_ViewMatrix", &m_ViewMatrix)))
		return E_FAIL;

	if (FAILED(m_pShaderCom->Bind_Matrix("g_ProjMatrix", &m_ProjMatrix)))
		return E_FAIL;

	if (FAILED(m_pShaderCom->Bind_RawValue("g_TextCoord", &m_TextCoord, sizeof(_float2))))
		return E_FAIL;

	if (FAILED(m_pShaderCom->Bind_RawValue("g_fAlpha", &m_fAlpha, sizeof(_float))))
		return E_FAIL;
#pragma region Base
	if (FAILED(m_pTransformCom->Bind_ShaderResource(m_pShaderCom, "g_WorldMatrix")))
		return E_FAIL;

	if (FAILED(m_pTextureCom->Bind_ShadeResource(m_pShaderCom, "g_Texture", 0)))
		return E_FAIL;

	if (FAILED(m_pShaderCom->Begin(0)))
		return E_FAIL;

	if (FAILED(m_pVIBufferCom->Bind_Buffers()))
		return E_FAIL;

	if (FAILED(m_pVIBufferCom->Render()))
		return E_FAIL;
#pragma endregion

#pragma region Guage
	m_pTransformCom->Set_Scaled(m_fSizeX - 142.f, m_fSizeY - 60.f, 1.f);

	m_pTransformCom->Set_State(CTransform::STATE_POSITION,
		XMVectorSet(m_fX + 31.f - m_fViewWidth * 0.5f, - m_fY - 3.f + m_fViewHeight * 0.5f, 0.f, 1.f));

	if (FAILED(m_pTransformCom->Bind_ShaderResource(m_pShaderCom, "g_WorldMatrix")))
		return E_FAIL;

	if (FAILED(m_pTexture_Guage->Bind_ShadeResource(m_pShaderCom, "g_Texture", 0)))
		return E_FAIL;

	if (FAILED(m_pShaderCom->Begin(5)))
		return E_FAIL;

	if (FAILED(m_pVIBufferCom->Bind_Buffers()))
		return E_FAIL;

	if (FAILED(m_pVIBufferCom->Render()))
		return E_FAIL;
#pragma endregion

#pragma region Icon
	m_pTransformCom->Set_Scaled(m_fSizeX - 400.f, m_fSizeY - 25.f, 1.f);

	m_pTransformCom->Set_State(CTransform::STATE_POSITION,
		XMVectorSet(m_fX - 170.f - m_fViewWidth * 0.5f, -m_fY - 5.f + m_fViewHeight * 0.5f, 0.f, 1.f));

	if (FAILED(m_pTransformCom->Bind_ShaderResource(m_pShaderCom, "g_WorldMatrix")))
		return E_FAIL;

	if (FAILED(m_pTexture_Icon->Bind_ShadeResource(m_pShaderCom, "g_Texture", (_uint)m_eIconType)))
		return E_FAIL;

	if (FAILED(m_pShaderCom->Begin(0)))
		return E_FAIL;

	if (FAILED(m_pVIBufferCom->Bind_Buffers()))
		return E_FAIL;

	if (FAILED(m_pVIBufferCom->Render()))
		return E_FAIL;
#pragma endregion

	return S_OK;
}

void CUI_BossHP::Appear(_float fTimeDelta)
{
	m_fAlpha += fTimeDelta;
	if (m_fAlpha >= 1.f)
	{
		m_fAlpha = 1.f;
		m_TextCoord.x += 2.f * fTimeDelta;

		if (m_TextCoord.x >= 1.f)
		{
			m_TextCoord.x = 1.f;
			m_isAppear = false;
		}
	}
}

void CUI_BossHP::Disappear(_float fTimeDelta)
{
	m_fAlpha -= 5.f * fTimeDelta;

	if (m_fAlpha <= 0.f)
	{
		m_fAlpha = 0.f;
		m_isDisappear = false;
		m_isActive = false;
	}
}

void CUI_BossHP::Start_Appear()
{
	if (!m_isAppear && !m_isDisappear)
	{
		m_TextCoord.x = 0.f;
		m_fAlpha = 0.f;
		m_isAppear = true;
		m_isActive = true;
	}
}

void CUI_BossHP::Start_Disappear()
{
	if (!m_isDisappear && !m_isAppear)
	{

		m_isDisappear = true;
	}
}

void CUI_BossHP::Damaged()
{
	m_fAlpha = 0.f;
	m_fDamagedTime = 0.f;
	m_isDamaged = true;
}

HRESULT CUI_BossHP::Ready_Components()
{
	/* FOR.Com_Shader */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxPosTex"),
		TEXT("Com_Shader"), reinterpret_cast<CComponent**>(&m_pShaderCom))))
		return E_FAIL;

	/* FOR.Com_Texture */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Texture_BossHPGuage"),
		TEXT("Com_Texture"), reinterpret_cast<CComponent**>(&m_pTextureCom))))
		return E_FAIL;

	/* FOR.Com_MaskTexture */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Texture_BossHPMask"),
		TEXT("Com_MaksTexture"), reinterpret_cast<CComponent**>(&m_pTexture_Guage))))
		return E_FAIL;

	/* FOR.Com_DamageTexture */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Texture_BossIcon"),
		TEXT("Com_IconTexture"), reinterpret_cast<CComponent**>(&m_pTexture_Icon))))
		return E_FAIL;

	/* FOR.Com_VIBuffer */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Rect"),
		TEXT("Com_VIBuffer"), reinterpret_cast<CComponent**>(&m_pVIBufferCom))))
		return E_FAIL;

	return S_OK;
}

CUI_BossHP* CUI_BossHP::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CUI_BossHP* pInstance = new CUI_BossHP(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CUI_BossHP"));
		Safe_Release(pInstance);
	}

	return pInstance;
}



CGameObject* CUI_BossHP::Clone(void* pArg)
{
	CUI_BossHP* pInstance = new CUI_BossHP(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CUI_BossHP"));
		Safe_Release(pInstance);
	}

	return pInstance;
}

void CUI_BossHP::Free()
{
	__super::Free();

	Safe_Release(m_pTexture_Guage);
	Safe_Release(m_pTexture_Icon);

	Safe_Release(m_pShaderCom);
	Safe_Release(m_pTextureCom);
	Safe_Release(m_pVIBufferCom);
}
