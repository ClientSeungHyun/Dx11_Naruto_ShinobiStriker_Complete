...
CState_Custom_Jump::CState_Custom_Jump(CFsm* pFsm, CPlayer* pPlayer)
	:CState{ pFsm }
	, m_pPlayer{ pPlayer }
{
}

HRESULT CState_Custom_Jump::Initialize(_uint iStateNum)
{
	m_iAnimation_Jump = m_pPlayer->Get_Model()->Get_AnimationIndex("CustomMan_Jump_Front", 2.6);
	m_iStateNum = iStateNum;

	return S_OK;
}

HRESULT CState_Custom_Jump::Start_State(void* pArg)
{
	...
	m_pPlayer->Get_EffectSoundCom()->Play3D(TEXT("Jump2.wav"), g_fVolume);

	return S_OK;
}

void CState_Custom_Jump::Update(_float fTimeDelta)
{
	Move(fTimeDelta);
...
void CState_Custom_Jump::End_State()
{

}

void CState_Custom_Jump::Set_JumpTargetPos()
{
...
void CState_Custom_Jump::Jump()
{
	_vector vJumpDir;
	_vector vJumpPower = XMVectorSet(1.f, 12.f, 1.f, 0.f);
...
void CState_Custom_Jump::Move(_float fTimeDelta)
{
...
CState_Custom_Jump* CState_Custom_Jump::Create(CFsm* pFsm, CPlayer* pPlayer, _uint iStateNum)
{
	CState_Custom_Jump* pInstance = new CState_Custom_Jump(pFsm, pPlayer);

	if (FAILED(pInstance->Initialize(iStateNum)))
	{
		MSG_BOX(TEXT("Failed to Created : CState_Custom_Jump"));
