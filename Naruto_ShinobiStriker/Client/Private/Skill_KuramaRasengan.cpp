...
CSkill_KuramaRasengan::CSkill_KuramaRasengan(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CSkill{ pDevice, pContext }
{
}

CSkill_KuramaRasengan::CSkill_KuramaRasengan(const CSkill_KuramaRasengan& Prototype)
	: CSkill{ Prototype }
{
}

HRESULT CSkill_KuramaRasengan::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CSkill_KuramaRasengan::Initialize(void* pArg)
{
	...
	m_pSocketMatrix = dynamic_cast<CMonster*>(m_pOwner)->Get_Model()->Get_BoneCombindTransformationMatrix_Ptr(m_szPositionBoneName);

	m_isActive = false;
	m_pColliderCom->IsActive(false);

	m_pMonnster = dynamic_cast<CMonster*>(m_pOwner);

	return S_OK;
}

void CSkill_KuramaRasengan::Priority_Update(_float fTimeDelta)
{
}

void CSkill_KuramaRasengan::Update(_float fTimeDelta)
{
	...
	if (!m_isShoot)
	{
		...
		_vector vPosition = WorldMatrix.r[3] + XMVector3Normalize(WorldMatrix.r[1]) * (1.5f + m_fCurrentTime);
...
void CSkill_KuramaRasengan::Late_Update(_float fTimeDelta)
{
	if (m_isShoot)
	{
		if (m_pNearTarget)
		{
			if(m_isMove)
				m_pTransformCom->Go_Straight(20.0f, fTimeDelta);
		}
...
HRESULT CSkill_KuramaRasengan::Render()
{
#ifdef _DEBUG
...
void CSkill_KuramaRasengan::OnCollisionEnter(CGameObject* pOther)
{
	if (pOther->Get_LayerTag() == TEXT("Layer_Player"))
	{
		...
		_float2 vForceBackPower = _float2(0.f, 0.f);
		pPlayer->Change_State(CPlayer::HIT_SPIN, &vForceBackPower);
...
void CSkill_KuramaRasengan::OnCollisionStay(CGameObject* pOther)
{
	if (pOther->Get_LayerTag() == TEXT("Layer_Player"))
	{
		Explosion();
...
void CSkill_KuramaRasengan::OnCollisionExit(CGameObject* pOther)
{
	if (pOther->Get_LayerTag() == TEXT("Layer_Player"))
	{
		CPlayer* pPlayer = dynamic_cast<CPlayer*>(pOther);
		...
		_float2 vForceBackPower = _float2(15.f, 10.f);
		pPlayer->Change_State(CPlayer::HIT_SPIN, &vForceBackPower);
...
void CSkill_KuramaRasengan::Appear()
{
	if (m_isActive)
		return;
	...
	m_pMassiveRasengan_Circle->Appear(6.f, 10.f);
	m_pMassiveRasengan_Spher->Appear(4.5f, 8.5f);

	m_pNearTarget = m_pGameInstance->Find_Player(m_iLevelIndex);
...
void CSkill_KuramaRasengan::DisAppear()
{
...
void CSkill_KuramaRasengan::Shoot()
{
	if (m_isShoot)
		return;
...
void CSkill_KuramaRasengan::Explosion()
{
	if (m_isExplosion)
		return;
...
HRESULT CSkill_KuramaRasengan::Ready_Components()
{
...
CSkill_KuramaRasengan* CSkill_KuramaRasengan::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CSkill_KuramaRasengan* pInstance = new CSkill_KuramaRasengan(pDevice, pContext);
...
CGameObject* CSkill_KuramaRasengan::Clone(void* pArg)
{
	CSkill_KuramaRasengan* pInstance = new CSkill_KuramaRasengan(*this);
...
void CSkill_KuramaRasengan::Free()
{
	__super::Free();
}
