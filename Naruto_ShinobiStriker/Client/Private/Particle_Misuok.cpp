#include "stdafx.h"
#include "..\Public\Particle_Misuok.h"

#include "GameInstance.h"
#include "Particle_Hit.h"
#include "ObjectPool.h"

CParticle_Misuok::CParticle_Misuok(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject{ pDevice, pContext }
{
}

CParticle_Misuok::CParticle_Misuok(const CParticle_Misuok& Prototype)
	: CGameObject{ Prototype }
{
}

HRESULT CParticle_Misuok::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CParticle_Misuok::Initialize(void* pArg)
{
...
void CParticle_Misuok::Priority_Update(_float fTimeDelta)
{

}

void CParticle_Misuok::Update(_float fTimeDelta)
{
	m_fCurrentTime += fTimeDelta;

	if (m_fCurrentTime >= 3.f)
	{
		CObjectPool<CParticle_Misuok>::Return_GameObject(this);
		m_isActive = false;
		return;
	}
...
void CParticle_Misuok::Late_Update(_float fTimeDelta)
{
...
HRESULT CParticle_Misuok::Render()
{
...
void CParticle_Misuok::Appear(_fvector vStartPos, _fvector vColor, _uint iNumParticle, _float fPower)
{
	if (m_isActive)
		false;
...
void CParticle_Misuok::Calculate_Gravity(_float fTimeDelta)
{

}

HRESULT CParticle_Misuok::Ready_Components()
{
	/* FOR.Com_Shader */
...
CParticle_Misuok* CParticle_Misuok::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CParticle_Misuok* pInstance = new CParticle_Misuok(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CParticle_Misuok"));
...
CGameObject* CParticle_Misuok::Clone(void* pArg)
{
	CParticle_Misuok* pInstance = new CParticle_Misuok(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CParticle_Misuok"));
		Safe_Release(pInstance);
	}
