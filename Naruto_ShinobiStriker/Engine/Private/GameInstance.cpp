...
CGameObject* CGameInstance::Get_CloneObject_AddLayer(_uint iLevelIndex, const _wstring& strLayerTag, const _wstring& strPrototypeTag, void* pArg)
{
...
void CGameInstance::Start_Lack(const _wstring& strTimerTag, _uint iLackCount)
{
	m_pTimer_Manager->Start_Lack(strTimerTag, iLackCount);
}
...
void CGameInstance::Set_IsRenderStaticLight(_bool isRender)
{
	m_pRenderer->Set_IsRenderStaticLight(isRender);
}

HRESULT CGameInstance::Load_StaticLightDepthTexture(const _wstring& strTextureFilePath)
{
	return m_pRenderer->Load_StaticLightDepthTexture(strTextureFilePath);
}
...
void CGameInstance::Set_StaticTransform(CPipeLine::D3DTRANSFORMSTATE eState, _fmatrix TransformMatrix)
{
	return m_pPipeLine->Set_StaticTransform(eState, TransformMatrix);
}

_float4x4 CGameInstance::Get_StaticTransform_Float4x4(CPipeLine::D3DTRANSFORMSTATE eState) const
{
	return m_pPipeLine->Get_StaticTransform_Float4x4(eState);
}
...
_vector CGameInstance::Get_LigthDepthPosition() const
{
	return m_pPipeLine->Get_LigthDepthPosition();
}

_vector CGameInstance::Get_LigthDepthAt() const
{
	return m_pPipeLine->Get_LigthDepthAt();
}

void CGameInstance::Set_LigthDepth(_fvector vShadowLightPosition, _fvector vShadowLightAt)
{
	m_pPipeLine->Set_LigthDepth(vShadowLightPosition, vShadowLightAt);
}
...
{
	m_pPicking_Manager->Add_ModelPickingList(pObj);
}

_vector CGameInstance::Get_RayPos()
{
	return m_pPicking_Manager->Get_RayPos();
}

void CGameInstance::Set_RayPos(_vector vRayPos)
{
	m_pPicking_Manager->Set_RayPos(vRayPos);
}

_vector CGameInstance::Get_RayDir()
{
	return m_pPicking_Manager->Get_RayDir();
}

void CGameInstance::Set_RayDir(_vector vRayDir)
{
	m_pPicking_Manager->Set_RayDir(vRayDir);
}

CGameObject* CGameInstance::Get_Player_Picking()
{
	return m_pPicking_Manager->Get_Player();
}

void CGameInstance::Set_Player_Picking(CGameObject* pPlayer)
{
	m_pPicking_Manager->Set_Player(pPlayer);
}
...
_bool CGameInstance::RayCast(_vector vRayPos, _vector vRayDir, _vector* vHitPos, _vector* vNormal, _float* fHitDistance)
{
	return m_pPhysX_Manager->RayCast(vRayPos, vRayDir, vHitPos, vNormal, fHitDistance);
}

_bool CGameInstance::RayCast_PlayerDown(_vector* vHitPos, _vector* vNormal, _float* fHitDistance)
{
	return m_pPhysX_Manager->RayCast_PlayerDown(vHitPos, vNormal, fHitDistance);
}

_bool CGameInstance::RayCast_PlayerLook(_vector* vHitPos, _vector* vNormal, _float* fHitDistance)
{
	return m_pPhysX_Manager->RayCast_PlayerLook(vHitPos, vNormal, fHitDistance);
}

void CGameInstance::Compute_Collision(_vector* vHitDir, _float* fHitLength)
{
	m_pPhysX_Manager->Compute_Collision(vHitDir, fHitLength);
}

HRESULT CGameInstance::Add_PhysX_Monster(CGameObject* pMonster)
{
	return m_pPhysX_Manager->Add_Monster(pMonster);
}
...
_float4x4 CGameInstance::Get_StaticLightViewMatrix() const
{
	return m_pLight_Manager->Get_StaticLightViewMatrix();
}
void CGameInstance::Set_StaticLightViewMatrix(_matrix vStaticViewMatirx)
{
	m_pLight_Manager->Set_StaticLightViewMatrix(vStaticViewMatirx);
}
#pragma endregion
...
HRESULT CGameInstance::Create_RenderTargetTexture(const _wstring& strFilePath, const _wstring& strTargetTag)
{
	return m_pTarget_Manager->Create_Texture(strFilePath, strTargetTag);
}

HRESULT CGameInstance::Load_RenderTargetTexture(const _wstring& strTargetTag, const _wstring& strTextureFilePath)
{
	return m_pTarget_Manager->Load_Texture(strTargetTag, strTextureFilePath);
}
