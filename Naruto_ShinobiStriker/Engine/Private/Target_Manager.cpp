...
HRESULT CTarget_Manager::Create_Texture(const _wstring& strFilePath, const _wstring& strTargetTag)
{
	CRenderTarget* pRenderTarget = Find_RenderTarget(strTargetTag);
	if (nullptr == pRenderTarget)
		return E_FAIL;

	return pRenderTarget->Create_Texture(strFilePath);
}

HRESULT CTarget_Manager::Load_Texture(const _wstring& strTargetTag, const _wstring& strTextureFilePath)
{
	CRenderTarget* pRenderTarget = Find_RenderTarget(strTargetTag);
	if (nullptr == pRenderTarget)
		return E_FAIL;

	return pRenderTarget->Load_Texture(strTextureFilePath);
}
