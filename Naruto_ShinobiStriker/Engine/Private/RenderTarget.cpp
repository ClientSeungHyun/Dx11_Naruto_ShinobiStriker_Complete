...
HRESULT CRenderTarget::Initialize(_uint iWidth, _uint iHeight, DXGI_FORMAT ePixelFormat, const _float4& vClearColor)
{
	...
	TextureDesc.Width  = m_iWidht = iWidth;
	TextureDesc.Height  = m_iHeight = iHeight;
	TextureDesc.MipLevels = 1;
	TextureDesc.ArraySize = 1;
	TextureDesc.Format = m_ePixelFormat = ePixelFormat;
...
{
	// 만들어둔 렌더 타겟을 쉐이더에 바인드 함
	return pShader->Bind_SRV(pConstantName, m_pSRV);
}

void CRenderTarget::Clear()
{
	// 렌더타겟에 그려둔 픽셀들을 원하는 색으로 초기화 함
	// 다시 그려지기 직전에 호출하기 -> 정보를 이용해야하기 때문
	m_pContext->ClearRenderTargetView(m_pRTV, (_float*)&m_vClearColor);
}

HRESULT CRenderTarget::Create_Texture(const _wstring& strFilePath)
{
	HRESULT result = SaveDDSTextureToFile(m_pContext, m_pTexture2D, strFilePath.c_str());
	return result;
}

HRESULT CRenderTarget::Load_Texture(const _wstring& strTextureFilePath)
{
	...
	HRESULT hr = CreateDDSTextureFromFile(m_pDevice, strTextureFilePath.c_str(), nullptr, &pSRV);
	if (S_OK == hr)
	{
		Safe_Release(m_pSRV);
		m_pSRV = pSRV;
