...
HRESULT CVIBuffer_Point_Instance::Initialize_Prototype(const CVIBuffer_Instancing::INSTANCE_PARTICLE_DESC& Desc)
{
...
HRESULT CVIBuffer_Point_Instance::Initialize(void * pArg)
{
	if (nullptr != pArg)
	{
		INSTANCE_PARTICLE_DESC* pDesc = static_cast<INSTANCE_PARTICLE_DESC*>(pArg);
...
#pragma region VERTEX_BUFFER
	...
	m_BufferDesc.Usage = D3D11_USAGE_DEFAULT; /* cpu gpu 모두에서 읽기 쓰기 가능 */
...
HRESULT CVIBuffer_Point_Instance::ReSetting_Instance(const INSTANCE_PARTICLE_DESC& Desc, void* pInstanceVertices)
{
	...
	// 바이트 크기만 다시 설정해주기
	m_iInstanceStride = sizeof(VTXEFFECTINSTANCE);
	m_InstanceBufferDesc.ByteWidth = m_iInstanceStride * m_iNumInstance;
	m_InstanceBufferDesc.Usage = D3D11_USAGE_DYNAMIC; /* 동적버퍼로 생성한다. */
	m_InstanceBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	m_InstanceBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	ZeroMemory(&m_InstanceInitialData, sizeof m_InstanceInitialData);
	m_InstanceInitialData.pSysMem = m_pInstanceVertices;
	ID3D11Buffer* pVBInstance = nullptr;

	if (FAILED(m_pDevice->CreateBuffer(&m_InstanceBufferDesc, &m_InstanceInitialData, &pVBInstance)))
		return E_FAIL;

	Safe_Release(m_pVBInstance);
	m_pVBInstance = pVBInstance;
...
void CVIBuffer_Point_Instance::Up(_float fTimeDelta)
{
	D3D11_MAPPED_SUBRESOURCE	SubResource{};
	...
	{
		_vector		vMoveDir = XMVectorSetW(XMLoadFloat4(&pVertices[i].vTranslation) - XMLoadFloat3(&m_vPivotPos), 0.f);
		vMoveDir = XMVectorSetY(vMoveDir, 1.f);
...
void CVIBuffer_Point_Instance::Explosion(_float fTimeDelta, _fvector vVelocity)
{
	D3D11_MAPPED_SUBRESOURCE	SubResource{};
	...
	for (size_t i = 0; i < m_iNumInstance; i++)
	{
		...
		XMStoreFloat4(&pVertices[i].vTranslation,
			XMLoadFloat4(&pVertices[i].vTranslation) + XMVector3Normalize(vMoveDir) * m_pSpeed[i] * vVelocity * fTimeDelta);
...
CVIBuffer_Point_Instance * CVIBuffer_Point_Instance::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, const CVIBuffer_Instancing::INSTANCE_PARTICLE_DESC& Desc)
{
