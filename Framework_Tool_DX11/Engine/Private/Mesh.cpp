...
{
	_matrix WroldMatrixInverse = XMMatrixInverse(nullptr, WorldMatrix);
	...
	for (size_t i = 0; i < (m_iNumIndices / 3); i++)
	{
		...
		{
			if (true == m_pGameInstance->isPicked_InLocalSpace(XMLoadFloat3(&m_pAnimVertices[m_pIndices[iNumIndices++]].vPosition),
				XMLoadFloat3(&m_pAnimVertices[m_pIndices[iNumIndices++]].vPosition),
				XMLoadFloat3(&m_pAnimVertices[m_pIndices[iNumIndices++]].vPosition),
				pOut))
				goto Compute_WorldPos;
		}
...
_bool CMesh::isPicking_Surface(const _fmatrix& WorldMatrix, _fvector fRayPos, _float3* pOut)
{
	_matrix WroldMatrixInverse = XMMatrixInverse(nullptr, WorldMatrix);
	m_pGameInstance->Transform_MouseRay_ToLocalSpace(WroldMatrixInverse);

	_vector vRayPos_InLocalSpace = XMVector3TransformCoord(fRayPos, WroldMatrixInverse);
	_vector vRayDir_InLocalSpace = XMVector3Normalize(m_pGameInstance->Get_RayDir_Local()) * -1.f;

	_uint		iNumIndices = { 0 };

	_float		fDist{};
	_float		fClosetDist = -1.f;
	...
	for (size_t i = 0; i < (m_iNumIndices / 3); i++)
	{
		if (nullptr != m_pVertices)
		{
			if (TRUE == TriangleTests::Intersects(vRayPos_InLocalSpace, vRayDir_InLocalSpace, XMLoadFloat3(&m_pVertices[m_pIndices[iNumIndices++]].vPosition),
				XMLoadFloat3(&m_pVertices[m_pIndices[iNumIndices++]].vPosition),
				XMLoadFloat3(&m_pVertices[m_pIndices[iNumIndices++]].vPosition), fDist))
			{
