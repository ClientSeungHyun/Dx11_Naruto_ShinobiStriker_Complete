...
{
	return m_Animations[iAnimationIndex]->Get_IsEnd();
}

_uint CModel::Get_Frame(_uint iAnimIndex)
{
	return m_Animations[iAnimIndex]->Get_CurrentFrame();
}
...
_bool CModel::isPicking(const _fmatrix& WorldMatrix, _float3* pOut)
{
	CMesh* pClosetMesh = m_Meshes[0];
	...
	//_vector vCameraPos_InLocal = XMVector3TransformCoord(m_pGameInstance->Get_CamPosition_Vector(), WroldMatrixInverse);
	...
	for (auto& Mesh : m_Meshes)
	{
		if (Mesh->isPicking(WorldMatrix, pOut))
		{
			//Mesh->isPicking_Surface(WorldMatrix, XMLoadFloat3(pOut), pOut);
			return true;
		}
	}
