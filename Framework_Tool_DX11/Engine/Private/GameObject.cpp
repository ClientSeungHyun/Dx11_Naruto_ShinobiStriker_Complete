...
void CGameObject::Compute_Distance_To_Camera()
{
	_vector vDistance = XMVector3Length(m_pGameInstance->Get_CamPosition_Vector() - Get_Position());
	XMStoreFloat(&m_fDistance_To_Camera, vDistance);
}

_float CGameObject::Compute_Distance(_fvector vPos)
{
	_vector vDistance = XMVector3Length(vPos - Get_Position());
	_float fDistance;
